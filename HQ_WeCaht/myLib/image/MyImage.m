...
+ (MyImage *)imageNamed:(NSString *)name {
...
+ (MyImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (MyImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (MyImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (CGRect)animatedImageContentsRectAtIndex:(NSUInteger)index{
    return CGRectMake(0, 0, [self animatedImageFrameAtIndex:index].size.width, [self animatedImageFrameAtIndex:index].size.height);
}
