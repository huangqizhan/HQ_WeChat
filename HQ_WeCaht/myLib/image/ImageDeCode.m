...
////IHDR: 图像头 是png 图片的编码格式
#pragma mark ------ IHDR: 图像头信息 --------
typedef struct {
...
//"fcTL" 块是 PNG 规范中定义的辅助块。它必须出现在它所适用的框架的 "IDAT" 或 "fdAT" 块之前,
typedef struct {
...
static bool yy_png_validate_animation_chunk_order(yy_png_chunk_info *chunks,  /* input */
    ...
    /*
     PNG at least contains 3 : IHDR, IDAT, IEND.
...
/**
 ...
 @param data 元数据
 @param info yy_png_info
 @param index index
 @param size output  size of frame data
...
///CPU读取数据是按照系统的位数读取的 （32位） 有些数据不够43位的 所以要对齐
...
///CGDataProviderCreateWithData   release data
...
@implementation ImageDeCode{
    pthread_mutex_t _lock;     ///递归锁
    BOOL _sourceTypeDetected;  ///是否已经检测类型
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    ImageDeCode *decoder = [[ImageDeCode alloc] initWithScale:scale];
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    YYImageType type = ImageTypeDetectType((__bridge CFDataRef)data);
...
+ (NSData *)encodeImageWithDecoder:(ImageDeCode *)decoder type:(YYImageType)type quality:(CGFloat)quality {
...
@implementation  UIImage (ImageCode)
...
- (BOOL)isDecodedForDisplay {
        if (self.images.count > 1 || [self isKindOfClass:[SpriteSheetImage class]]) return YES;
...
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[MyImage class]]) {
        MyImage *image = (id)self;
