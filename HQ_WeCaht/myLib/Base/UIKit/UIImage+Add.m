...
YYSSSYNTH_DUMMY_CLASS(UIImage_Add)

/////GIF 每一帧的停顿时间
static NSTimeInterval GetGIFImageFrameDelayAtIndex(CGImageSourceRef source,size_t index ){
...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = GetGIFImageFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+(UIImage *)combineWithImages:(NSArray *)images orientation:(HQImageCombineType)orientation{
    ...
    for (id image in images) {
        ...
        if(orientation == HQImageCombineHorizental){//横向
            maxWidth += size.width;
...
- (UIImage *)shrinkImageWithCapInsets:(UIEdgeInsets)capInsets actualSize:(CGSize)actualSize{//默认拉伸好了 暂时不处理平铺的情况
    ...
    if(actualSize.width < self.size.width){
        ...
        if(imageArray.count > 0){
            newAllImage = [UIImage combineWithImages:imageArray orientation:HQImageCombineHorizental];
    ...
    if(actualSize.height < self.size.height){
        ...
        //拼接
        if(imageArray.count > 0){
            newAllImage = [UIImage combineWithImages:imageArray orientation:HQImageCombineVertical];
            return newAllImage;
        }
    }
