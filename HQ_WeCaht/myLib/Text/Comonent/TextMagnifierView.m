...
/**
 圆型放大镜
 */
@interface TextMagnifierCartView : TextMagnifierView

@end

@implementation  TextMagnifierCartView{
...
- (TextMagnifierType)type {
    return TextMagnifierTypeCaret;
}
- (UIImage *)snapshot {
...
+ (UIImage *)coverImage {
    ...
    dispatch_once(&onceToken, ^{
        ...
        CGPathRef strokePath = CGPathCreateWithEllipseInRect(TextCGRectPixelHalf(rect), NULL);
        ...
        CGContextSaveGState(context); {
            ...
            CGContextSetLineWidth(context, TextCGFloatFromPixel(1));
...
/**
 矩形放大镜
 */
@interface TextMagnifierRanged : TextMagnifierView

@end

@implementation TextMagnifierRanged{
    UIImageView *_contentView;
    UIImageView *_coverView;
}
#define kMultiple 1.2
#define kSize CGSizeMake(141, 60)
#define kPadding TextCGFloatPixelHalf(6.0)
#define kRadius 6.0
#define kHeight 32.0
#define kArrow 14.0
...
- (TextMagnifierType)type {
    return TextMagnifierTypeRanged;
}
...
+ (UIImage *)coverImage {
    ...
    dispatch_once(&onceToken, ^{
        ...
        CGPathMoveToPoint(arrowPath, NULL, size.width / 2 - kArrow, TextCGFloatPixelFloor(kPadding) + kHeight);
        CGPathAddLineToPoint(arrowPath, NULL, size.width / 2 + kArrow, TextCGFloatPixelFloor(kPadding) + kHeight);
...
@implementation TextMagnifierView

+ (id)magnifierWithType:(TextMagnifierType)type {
    switch (type) {
        case TextMagnifierTypeCaret :return [TextMagnifierCartView new];
        case TextMagnifierTypeRanged :return [TextMagnifierRanged new];
    }
    return nil;
}
- (id)initWithFrame:(CGRect)frame {
    // class cluster
    if ([self isMemberOfClass:[TextMagnifierView class]]) {
