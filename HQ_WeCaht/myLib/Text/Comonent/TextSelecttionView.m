...
@implementation TextSelecttionGrabberDot
...
@implementation TextSelectionGrabber
- (instancetype) initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (!self) return nil;
    _dot = [[TextSelecttionGrabberDot alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    return self;
}

- (void)setDotDirection:(TextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == TextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == TextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == TextDirectionBottom) {
        frame.origin.y = self.bounds.size.height - ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == TextDirectionLeft) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == TextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == TextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == TextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == TextDirectionLeft) {
...
@interface TextSelecttionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) TextSelectionGrabber *startGrabber;
@property (nonatomic, strong) TextSelectionGrabber *endGrabber;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [TextSelectionGrabber new];
    _startGrabber.dotDirection = TextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [TextSelectionGrabber new];
    _endGrabber.dotDirection = TextDirectionBottom;
...
- (void)_startBlinks {
    [_caretTimer invalidate];
    if (_caretVisible) {
        _caretTimer = [NSTimer timerWithTimeInterval:kBlinkDuration target:[TextWeakProxy proxyWithTarget:self] selector:@selector(_doBlink) userInfo:nil repeats:YES];
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? TextDirectionRight : TextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? TextDirectionLeft : TextDirectionBottom;
    }
}
...
- (CGRect)_standardCaretRect:(CGRect)caretRect {
    ...
    caretRect = TextCGRectPixelRound(caretRect);
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(TextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isStartGrabberContainsPoint:(CGPoint)point {
    ...
    if (CGRectIntersectsRect(startRect, endRect)) {
        CGFloat distStart = TextCGPointGetDistanceToPoint(point, TextCGRectGetCenter(startRect));
        CGFloat distEnd = TextCGPointGetDistanceToPoint(point, TextCGRectGetCenter(endRect));
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (TextSelectionRect *rect in _selectionRects) {
