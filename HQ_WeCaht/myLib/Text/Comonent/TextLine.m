...
- (void)reloadBounds{
    ...
    if (!_CTLine) return;
    CFArrayRef runs = CTLineGetGlyphRuns(_CTLine);
    NSUInteger runNum = CFArrayGetCount(runs);
    if(runNum == 0) return;
    ...
    for (int i = 0; i < runNum; i++) {
        ...
        NSDictionary *atts = (id)CTRunGetAttributes(run);
        TextAttachment *attachment = atts[TextAttachmentAttributeName];
        if (attachment) {
            ...
            NSRange runRange = TextNSRangeFromCFRange(CTRunGetStringRange(run));
