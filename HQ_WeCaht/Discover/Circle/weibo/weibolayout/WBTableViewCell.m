...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    line.size = CGSizeMake(self.width, TextCGFloatFromPixel(1));
...
@implementation WBProfileView{
    ///如果点击到需要处理的区域 就不会传到下一级的响应者
    BOOL _trackingTouch;
}
- (instancetype)initWithFrame:(CGRect)frame {
    if (frame.size.width == 0 && frame.size.height == 0) {
    ...
    avatarBorder.borderWidth = TextCGFloatFromPixel(1);
    ...
    _nameLabel.lineBreakModel = NSLineBreakByClipping;
    _nameLabel.textVerticalAlignment = TextVerticalAlignmentCenter;
    [self addSubview:_nameLabel];
    
    _sourceLabel = [HQLabel new];
...
- (void)setVerifyType:(WBUserVerifyType)verifyType {
    ...
    switch (verifyType) {
        case WBUserVerifyTypeStandard: {
            ...
            _avatarBadgeView.image = [WBHelper imageWithNamed:@"avatar_vip"];
        } break;
        case WBUserVerifyTypeClub: {
            _avatarBadgeView.hidden = NO;
            _avatarBadgeView.image = [WBHelper imageWithNamed:@"avatar_grassroot"];
        } break;
        default: {
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _label = [HQLabel new];
    _label.textVerticalAlignment = TextVerticalAlignmentCenter;
    ...
    self.layer.borderWidth = TextCGFloatFromPixel(1);
    self.layer.borderColor = [UIColor colorWithWhite:0.000 alpha:0.070].CGColor;
    return self;
}
- (void)setWithLayout:(WBLayout *)layout isRetweet:(BOOL)isRetweet {
    WBPageInfo *pageInfo = isRetweet ? layout.wbModel.retweetedStatus.pageInfo : layout.wbModel.pageInfo;
    if (!pageInfo) return;
    ...
    switch (isRetweet ? layout.retweetCardType : layout.cardType) {
        ...
        case WBStatusCardTypeVideo: {
            ...
            [_button setImage:[WBHelper imageWithNamed:@"multimedia_videocard_play"] forState:UIControlStateNormal];
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _repostButton.size = CGSizeMake(TextCGFloatPixelRound(self.width / 3.0), self.height);
    ...
    _commentButton.size = CGSizeMake(TextCGFloatPixelRound(self.width / 3.0), self.height);
    _commentButton.left = TextCGFloatPixelRound(self.width / 3.0);
    ...
    _likeButton.size = CGSizeMake(TextCGFloatPixelRound(self.width / 3.0), self.height);
    _likeButton.left = TextCGFloatPixelRound(self.width / 3.0 * 2.0);
    [_likeButton setBackgroundImage:[UIImage imageWithColor:kWBCellHighlightColor] forState:UIControlStateHighlighted];
    
    _repostImageView = [[UIImageView alloc] initWithImage:[WBHelper imageWithNamed:@"timeline_icon_retweet"]];
    _repostImageView.centerY = self.height / 2;
    [_repostButton addSubview:_repostImageView];
    _commentImageView = [[UIImageView alloc] initWithImage:[WBHelper imageWithNamed:@"timeline_icon_comment"]];
    _commentImageView.centerY = self.height / 2;
    [_commentButton addSubview:_commentImageView];
    _likeImageView = [[UIImageView alloc] initWithImage:[WBHelper imageWithNamed:@"timeline_icon_unlike"]];
    _likeImageView.centerY = self.height / 2;
    [_likeButton addSubview:_likeImageView];
    
    _repostLabel = [HQLabel new];
    _repostLabel.userInteractionEnabled = NO;
    _repostLabel.height = self.height;
    _repostLabel.textVerticalAlignment = TextVerticalAlignmentCenter;
    ...
    _commentLabel = [HQLabel new];
    _commentLabel.userInteractionEnabled = NO;
    _commentLabel.height = self.height;
    _commentLabel.textVerticalAlignment = TextVerticalAlignmentCenter;
    ...
    _likeLabel = [HQLabel new];
    _likeLabel.userInteractionEnabled = NO;
    _likeLabel.height = self.height;
    _likeLabel.textVerticalAlignment = TextVerticalAlignmentCenter;
    ...
    _line1.size = CGSizeMake(TextCGFloatFromPixel(1), self.height);
    ...
    _line2.size = CGSizeMake(TextCGFloatFromPixel(1), self.height);
    _line2.left = _commentButton.right;
    
    _topLine = [CALayer layer];
    _topLine.size = CGSizeMake(self.width, TextCGFloatFromPixel(1));
...
- (void)setWithLayout:(WBLayout *)layout {
    ...
    _likeImageView.image = layout.wbModel.attitudesStatus ? [self likeImage] : [self unlikeImage];
}
- (UIImage *)likeImage {
    static UIImage *img;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        img = [WBHelper imageWithNamed:@"timeline_icon_like"];
    });
    return img;
}
...
- (UIImage *)unlikeImage {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        img = [WBHelper imageWithNamed:@"timeline_icon_unlike"];
    });
    return img;
}
- (void)adjustImage:(UIImageView *)image label:(HQLabel *)label inButton:(UIButton *)button {
    ...
    image.centerX = TextCGFloatPixelRound(paddingSide + imageWidth / 2);
    label.right = TextCGFloatPixelRound(button.width - paddingSide);
}
- (void)setLiked:(BOOL)liked withAnimation:(BOOL)animation {
//    WBSLayout *layout = _cell.statusView.layout;
...
//        }
    ...
    _imageView.image = [WBHelper imageWithNamed:@"timeline_icon_locate"];
    _imageView.hidden = YES;
    [self addSubview:_imageView];
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _titleView = [WBTitleView new];
    _titleView.hidden = YES;
    [_contentView addSubview:_titleView];
    
    _profileView = [WBProfileView new];
    ...
    [_menuButton setImage:[WBHelper imageWithNamed:@"timeline_icon_more"] forState:UIControlStateNormal];
    [_menuButton setImage:[WBHelper imageWithNamed:@"timeline_icon_more_highlighted"] forState:UIControlStateHighlighted];
...
//    }];
    ...
    _textLabel = [HQLabel new];
    _textLabel.left = kWBCellPadding;
    _textLabel.width = kWBCellContentWidth;
    _textLabel.textVerticalAlignment = TextVerticalAlignmentTop;
...
//        }
    ...
    _retweetTextLabel = [HQLabel new];
    _retweetTextLabel.left = kWBCellPadding;
    _retweetTextLabel.width = kWBCellContentWidth;
    _retweetTextLabel.textVerticalAlignment = TextVerticalAlignmentTop;
...
//        }
    ...
    for (int i = 0; i < 9; i++) {
        ImageControll *imageView = [ImageControll new];
        imageView.size = CGSizeMake(100, 100);
        imageView.hidden = YES;
        imageView.clipsToBounds = YES;
        imageView.backgroundColor = kWBCellHighlightColor;
        imageView.exclusiveTouch = YES;
//        imageView.touchBlock = ^(ImageControll *view, UIGestureRecognizerState state, NSSet *touches, UIEvent *event) {
...
- (void)setLayout:(WBLayout *)layout {
    ...
    /// 圆角头像
    [_profileView.avatarView setImageWithURL:layout.wbModel.user.avatarLarge placeholder:nil                        options:kNilOptions
        manager:[WBHelper avatarImageManager] ///< 圆角头像manager，内置圆角处理
    ...
    _profileView.verifyType = layout.wbModel.user.userVerifyType;
    _profileView.height = layout.profileHeight;
    _profileView.top = top;
    top += layout.profileHeight;
    
    NSURL *picBg = [WBHelper defaultURLForImageURL:layout.wbModel.picBg];
...
- (void)_setImageViewWithTop:(CGFloat)imageTop isRetweet:(BOOL)isRetweet {
    ...
    NSArray *pics = isRetweet ? _layout.wbModel.retweetedStatus.pics : _layout.wbModel.pics;
        ...
        if (i >= picsCount) {
            ...
            switch (pic.largest.badgeType) {
                ...
                case WBPictureBadgeTypeLong: {
                    badge.layer.contents = (__bridge id)([WBHelper imageWithNamed:@"timeline_image_longimage"].CGImage);
                    badge.hidden = NO;
                } break;
                case WBPictureBadgeTypeGIF: {
                    badge.layer.contents = (__bridge id)([WBHelper imageWithNamed:@"timeline_image_gif"].CGImage);
                    badge.hidden = NO;
                } break;
            }

            @weakify(imageView);
            [imageView.layer setImageWithURL:pic.bmiddle.url placeholder:nil options:YYWebImageOptionAvoidSetImage  completion:^(UIImage *image, NSURL *url, YYWebImageFromType from, YYWebImageStage stage, NSError *error) {
                @strongify(imageView);
                if (!imageView) return;
                if (image && stage == YYWebImageStageFinished) {
                    ...
                    ((ImageControll *)imageView).image = image;
