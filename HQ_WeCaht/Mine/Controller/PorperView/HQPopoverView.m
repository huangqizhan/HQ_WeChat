...
- (void)layoutSubviews {
    ...
    _tableView.frame = CGRectMake(0, _isUpward ? PopoverViewArrowHeight : 0, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - PopoverViewArrowHeight);
}
#pragma mark - Setter
...
- (void)setStyle:(HQPopoverActionStyle )style {
    _style = style;
    _tableView.separatorColor = [HQPopoverViewCell bottomLineColorForStyle:_style];
...
- (void)initialize{
    _actions = @[];
    _isUpward = YES;
    _style = HQPopoverActionDefauleStyle;
    self.backgroundColor = [UIColor whiteColor];
    ...
    [_tableView registerClass:[HQPopoverViewCell class] forCellReuseIdentifier:@"HQPopoverViewCell"];
    _tableView.separatorColor = [HQPopoverViewCell bottomLineColorForStyle:_style];
...
- (void)showToPoint:(CGPoint)toPoint{
    ...
    CGFloat minHorizontalEdge = PopoverViewMargin + cornerRadius + arrowWidth/2 + 2;
    ...
    CGFloat currentH = _tableView.contentSize.height + PopoverViewArrowHeight;
    
    // 限制最高高度, 免得选项太多时超出屏幕
    CGFloat maxHeight = _isUpward ? (_windowHeight - toPoint.y - PopoverViewMargin) : (toPoint.y - CGRectGetHeight([UIApplication sharedApplication].statusBarFrame));
    ...
    // x: 窗口靠左
    if (toPoint.x <= currentW/2 + PopoverViewMargin) {
        currentX = PopoverViewMargin;
    }
    // x: 窗口靠右
    if (_windowWidth - toPoint.x <= currentW/2 + PopoverViewMargin) {
        currentX = _windowWidth - PopoverViewMargin - currentW;
    }
    // y: 箭头向下
    if (!_isUpward) {
    ...
    CGFloat maskTop = _isUpward ? PopoverViewArrowHeight : 0; // 顶部Y值
    CGFloat maskBottom = _isUpward ? currentH : currentH - PopoverViewArrowHeight; // 底部Y值
    ...
    // 箭头向上时的箭头位置
    if (_isUpward) {
        [maskPath addLineToPoint:CGPointMake(arrowPoint.x - arrowWidth/2, PopoverViewArrowHeight)];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowCornerRadius, arrowCornerRadius)
                         controlPoint:CGPointMake(arrowPoint.x - arrowWidth/2 + arrowBottomCornerRadius, PopoverViewArrowHeight)];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowCornerRadius, arrowCornerRadius)
                         controlPoint:arrowPoint];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowWidth/2, PopoverViewArrowHeight)
                         controlPoint:CGPointMake(arrowPoint.x + arrowWidth/2 - arrowBottomCornerRadius, PopoverViewArrowHeight)];
    }
    // 右上圆角
    ...
    // 箭头向下时的箭头位置
    if (!_isUpward) {
        [maskPath addLineToPoint:CGPointMake(arrowPoint.x + arrowWidth/2, currentH - PopoverViewArrowHeight)];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowCornerRadius, currentH - arrowCornerRadius)
                         controlPoint:CGPointMake(arrowPoint.x + arrowWidth/2 - arrowBottomCornerRadius, currentH - PopoverViewArrowHeight)];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowCornerRadius, currentH - arrowCornerRadius)
                         controlPoint:arrowPoint];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowWidth/2, currentH - PopoverViewArrowHeight)
                         controlPoint:CGPointMake(arrowPoint.x - arrowWidth/2 + arrowBottomCornerRadius, currentH - PopoverViewArrowHeight)];
    }
    // 左下圆角
...
- (CGFloat)calculateMaxWidth {
    CGFloat maxWidth = 0.f, titleLeftEdge = 0.f, imageWidth = 0.f, imageMaxHeight = PopoverViewCellHeight - PopoverViewCellVerticalMargin*2;
    CGSize imageSize = CGSizeZero;
    UIFont *titleFont = [HQPopoverViewCell titleFont];
    for (HQPopoverAction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;
...
#pragma GCC diagnostic pop
    ...
    if (maxWidth > CGRectGetWidth(_keyWindow.bounds) - PopoverViewMargin*2) {
        maxWidth = CGRectGetWidth(_keyWindow.bounds) - PopoverViewMargin*2;
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return PopoverViewCellHeight;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    HQPopoverViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HQPopoverViewCell"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [UIView animateWithDuration:0.25f animations:^{
        ...
        HQPopoverAction *action = _actions[indexPath.row];
        action.hander ? action.hander(action) : NULL;
        _actions = nil;
        [_shadeView removeFromSuperview];
        [self removeFromSuperview];
    }];
