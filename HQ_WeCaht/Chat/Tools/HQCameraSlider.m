...
@interface HQCameraSlider (){
    
    CGFloat gap;
    CGFloat inverseGap;
    
}
@property (nonatomic, assign) HQSliderDirection direction;//方向（横或竖）
...
@implementation HQCameraSlider


- (id)initWithFrame:(CGRect)frame{
    return [self initWithFrame:frame direction:HQSliderDirectionHorizonal];
}

- (id)initWithFrame:(CGRect)frame direction:(HQSliderDirection)direction {
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat startLineX = (_direction == HQSliderDirectionHorizonal ? gap : (self.frame.size.width - _lineWidth) / 2);
    CGFloat startLineY = (_direction == HQSliderDirectionHorizonal ? (self.frame.size.height - _lineWidth) / 2 : gap);
    
    CGFloat endLineX = (_direction == HQSliderDirectionHorizonal ? self.frame.size.width - gap : (self.frame.size.width - _lineWidth) / 2);
    CGFloat endLineY = (_direction == HQSliderDirectionHorizonal ? (self.frame.size.height - _lineWidth) / 2 : self.frame.size.height- gap);
    ...
    CGContextSetLineWidth(context, _lineWidth);//线的宽度
    CGFloat slidedLineX = (_direction == HQSliderDirectionHorizonal ? MAX(gap, (_scaleNum * self.frame.size.width - gap)) : startLineX);
    CGFloat slidedLineY = (_direction == HQSliderDirectionHorizonal ? startLineY : MAX(gap, (_scaleNum * self.frame.size.height - gap)));
    CGContextMoveToPoint(context, startLineX, startLineY);//起点
    ...
    CGFloat circleX = (_direction == HQSliderDirectionHorizonal ? MAX(_circleRadius + penWidth, slidedLineX - penWidth - inverseGap) : startLineX);
    CGFloat circleY = (_direction == HQSliderDirectionHorizonal ? startLineY : MAX(_circleRadius + penWidth, slidedLineY - penWidth - inverseGap));
...
- (void)updateTouchPoint:(NSSet*)touches {
    ...
    self.scaleNum = (_direction == HQSliderDirectionHorizonal ? touchPoint.x : touchPoint.y) / (_direction == HQSliderDirectionHorizonal ? self.frame.size.width : self.frame.size.height);
}
