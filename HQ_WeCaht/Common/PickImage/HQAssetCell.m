...
@implementation HQAssetCell

- (void)setModel:(HQAssetModel *)model {
    _model = model;
    if (iOS8Later) {
...
- (void)setType:(HQAssetCellType)type {
    _type = type;
    if (type == HQAssetCellTypePhoto || type == HQAssetCellTypeLivePhoto || (type == HQAssetCellTypePhotoGif && !self.allowPickingGif)) {
...
- (void)fetchBigImage {
    _bigImageRequestID = [[HQImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded,id identifier) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (HQPickerProcessView *)progressView {
    if (_progressView == nil) {
        _progressView = [[HQPickerProcessView alloc] init];
...
@implementation HQAlbumCell

- (void)setModel:(HQAlbumModel *)model {
    _model = model;
    ...
    [[HQImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (UIImageView *)posterImageView {
    if (_posterImageView == nil) {
        ...
        posterImageView.frame = CGRectMake(15, 10, 70, 70);
...
- (UILabel *)titleLabel {
    if (_titleLabel == nil) {
        ...
        titleLabel.frame = CGRectMake(100, 10, self.tz_width - 80 - 50, self.tz_height);
...
@implementation HQAssetCameraCell
