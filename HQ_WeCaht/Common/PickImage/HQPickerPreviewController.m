...
#import "HQPhotoPrevireViewCell.h"
#import "HQAssetModel.h"



@interface HQPickerPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate>{
...
@implementation HQPickerPreviewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [HQImageManager manager].shouldFixOrientation = YES;
    __weak typeof(self) weakSelf = self;
    HQPickerImageViewController *_tzImagePickerVc = (HQPickerImageViewController *)weakSelf.navigationController;
...
- (void)configBottomToolBar {
    ...
    HQPickerImageViewController *_tzImagePickerVc = (HQPickerImageViewController *)self.navigationController;
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[HQPhotoPrevireViewCell class] forCellWithReuseIdentifier:@"HQPickerProcessView"];
}
- (void)configCropView {
    HQPickerImageViewController *_tzImagePickerVc = (HQPickerImageViewController *)self.navigationController;
    if (!_tzImagePickerVc.showSelectBtn && _tzImagePickerVc.allowCrop) {
        ...
        [HQImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)select:(UIButton *)selectButton {
    HQPickerImageViewController *_tzImagePickerVc = (HQPickerImageViewController *)self.navigationController;
    HQAssetModel *model = _models[_currentIndex];
        ...
        for (HQAssetModel *model_item in selectedModels) {
            if ([[[HQImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[HQImageManager manager] getAssetIdentifier:model_item.asset]]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    HQAssetModel *model = selectedModelsTmp[i];
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        HQPhotoPrevireViewCell *cell = (HQPhotoPrevireViewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        UIImage *cropedImage = [HQImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [HQImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    HQPhotoPrevireViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HQPickerProcessView" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[HQPhotoPrevireViewCell class]]) {
        [(HQPhotoPrevireViewCell *)cell recoverSubviews];
    }
}
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    if (![[HQImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
...
- (void)showPhotoBytes {
    [[HQImageManager manager] getPhotosBytesWithArray:@[_models[_currentIndex]] completion:^(NSString *totalBytes) {
