...
@interface HTTPURLSessionManager ()
...
+ (instancetype)shareChatSession{
    static HTTPURLSessionManager *manager = nil;
    ...
    dispatch_once(&onceToken, ^{
        ...
        manager = [[HTTPURLSessionManager alloc] initWithBaseURL:[NSURL URLWithString:@"https://sslqa.haimianyikao.com"] sessionConfiguration:config];
    });
    return manager;
}
...
- (instancetype)initWithBaseURL:(NSURL *)url
    ...
    self.requestSerializer = [JSONHttpRequestSerializer serializer];
    self.responseSerializer = [JSONResponseSerializer serializer];
    
    return self;
}

- (void)setRequestSerializer:(HttpRequestSerializer<URLRequestSerialization> *)requestSerializer{
    NSParameterAssert(requestSerializer);
    _requestSerializer = requestSerializer;
    
}
- (void)setResponseSerializer:(HTTPResponseSerializer<URLResponseSerialization> *)responseSerializer{
...
#pragma clang diagnostic pop
    ...
    dataTask = [self downloadTaskWithRequest:request progress:downloadProgress  destination:destination completionHandler:completionHandler];
    [dataTask resume];
    return dataTask;
}
...
- (instancetype)initWithCoder:(NSCoder *)decoder {
    ...
    SecurityPolicy *decodedPolicy = [decoder decodeObjectOfClass:[SecurityPolicy class] forKey:NSStringFromSelector(@selector(securityPolicy))];
    if (decodedPolicy) {
...
- (instancetype)copyWithZone:(NSZone *)zone {
    HTTPURLSessionManager *HTTPClient = [[[self class] allocWithZone:zone] initWithBaseURL:self.baseURL sessionConfiguration:self.session.configuration];
