...
@interface ZbarWraper ()<ZBarReaderViewDelegate>

@property (nonatomic, strong) ZBarReaderView *readerView;
@property (nonatomic, copy) void (^success)(NSArray<ZbarResult*> *result);


@end

@implementation ZbarWraper
...
- (instancetype)initWithPreView:(UIView*)preView barCodeType:(zbar_symbol_type_t)barCodeType block:(void(^)(NSArray<ZbarResult*> *result))block{
    if (self = [super init]){
        ...
        _readerView.readerDelegate =self;
         [_readerView.device lockForConfiguration:nil];
        _readerView.device.focusMode = AVCaptureFocusModeContinuousAutoFocus;
        [_readerView.device unlockForConfiguration];
        //关闭闪光灯
...
- (void)changeBarCode:(zbar_symbol_type_t)zbarFormat{
    ...
    [scanner setSymbology: zbarFormat config: ZBAR_CFG_ENABLE to: 0];
}
...
- (void) readerView: (ZBarReaderView*) readerView  didReadSymbols: (ZBarSymbolSet*) symbols  fromImage: (UIImage*) image{
    ...
    for (symbol in symbols) {
        ...
        ZbarResult *result = [ZbarResult new];
